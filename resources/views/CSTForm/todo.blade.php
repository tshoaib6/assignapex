...
<div class="card">
 ...
 <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                $status = ($step == 12 && $statusValue == 4) ? ['label' => 'Pending', 'class' => 'warning'] :
                ($statusMap[$statusValue] ?? ['label' => 'Unknown', 'class' => 'secondary']);

                @endphp
                <tr>
                    <td>{{ $data->unique_request_id }}</td>
                    ...
                    <td>
                        @if(Auth::id() == $data->user->id && $data->status == 1)
                        ...
                        @endif
                        <a href="{{ route('cstform.view', $data->id) }}" target="_blank">
                ...
                $status = $statusMap[$checklist->status] ?? ['label' => 'Unknown', 'class' => 'secondary'];
                @endphp
                <tr>
                    <td>{{ $checklist->cstRequest->unique_request_id }}</td>
                    <td>{{ $checklist->cstRequest->user->name ?? 'N/A' }}</td>
                    <td><span class="badge bg-{{ $status['class'] }}">{{ $status['label'] }}</span></td>
                    <td>
                        @if($position === 'Team Lead')
                        <a href="{{ route('logfile.test', ['id' => $checklist->cstRequest->id]) }}"
                            class="btn btn-info btn-sm">
                            <i class="fa fa-cogs me-1"></i> Process
                        </a>
                        @else
                        <a href="{{ route('post.checklist', ['id' => $checklist->id]) }}" class="btn btn-info btn-sm">
                            <i class="fa fa-cogs me-1"></i> Process
                        </a>
                        @endif
                    </td>
                    <td> <a href="{{ route('cstform.view', $checklist->cstRequest->id) }}" target="_blank">
                            <i class="fa fa-external-link-alt text-info"></i> Report
                        </a>
                    </td>
                </tr>
