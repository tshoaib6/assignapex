...
@section('title', 'Edit Check Point')
...
<h1 class="page-header mb-4">
    Edit Check Point <small class="text-muted">Update post-processor check point details</small>
</h1>

<div class="card">
    <div class="card-body">
        <form action="{{ route('post-processor-checklists.update', $checklist->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <label class="form-label">Section <span class="text-danger">*</span></label>
                <input type="text" name="section" value="{{ $checklist->section }}" class="form-control" required>
            </div>

            <div class="mb-3">
                <label class="form-label">Parent Title</label>
                <input type="text" name="parent_title" value="{{ $checklist->parent_title }}" class="form-control">
            </div>

            <div class="mb-3">
                <label class="form-label">Check Point <span class="text-danger">*</span></label>
                <input type="text" name="check_point" value="{{ $checklist->check_point }}" class="form-control" required>
            </div>
